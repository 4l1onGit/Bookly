...
class UserAPIController extends AbstractFOSRestController
{
    ...
    #[Rest\Get('api/v1/users/{id}/reviews', name: 'user_a_reviews')]
    ...
    #[Rest\Put('api/v1/users/{u_id}/reviews/{r_id}', name: 'users_review_update')]
    public function updateUserReview($u_id, $r_id, Request $request) {
        $user = $this->userRepo->find($u_id);
        if($this->getUser() !== $user && !$this->isGranted('ROLE_ADMIN')) {
            $view = $this->view(['error' => 'You are not allowed to view this user'], 403);
        } else {
